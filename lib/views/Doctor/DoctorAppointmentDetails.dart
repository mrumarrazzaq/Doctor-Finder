...
class _DoctorAppointmentDetailsState extends State<DoctorAppointmentDetails> {
  ...
  fetchAppointmentDetails() async{
    print(widget.id);
    final response = await get(Uri.parse("$SERVER_ADDRESS/api/appointmentdetail?id=${widget.id}&type=2"));
    print(Uri.parse("$SERVER_ADDRESS/api/appointmentdetail?id=${widget.id}&type=2"));
                  ...
                  children: [
                    ClipRRect(
                      ...
                      child: CachedNetworkImage(
                        imageUrl: Uri.encodeFull(list.user_Image!),
                        height: 75,
                        width: 75,
                        fit: BoxFit.cover,
  ...
  changeStatus(status) async{
    dialog();
    final response = await post(Uri.parse("$SERVER_ADDRESS/api/appointmentstatuschange?app_id=${widget.id}&status=$status"));
    if(response.statusCode == 200){
      final jsonResponse = jsonDecode(response.body);
      if(jsonResponse['success'].toString() == "1"){
        Navigator.pop(context);
